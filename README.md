...
**A pure-Rust MIDI synthesizer, developed alongside twenty-four reproducible albums
generated by five language models.**

The albums were produced by language models writing Python that emits MIDI. Their
briefs use broad musical idioms as reference points; they are intended as new
compositions, not reconstructions of named works. That is a statement of intent,
not a guarantee against incidental similarity. The synth's optional recorded
instrument material is separately licensed and documented. The generators and
the MIDI they produce are committed as reproducible source; rendered audio is
build output you regenerate with one command
...
is the source of truth. For a fixed executable and input, the deterministic
renderer produces stable output. Catalog renders are loudness-normalized to
−18 LUFS with a −1 dBTP true-peak ceiling and carry R128 replay-gain tags.
...
- **Every Voice Forward** — *progressive pop / cinematic / instrumental*, 21:24 ·
  5 tracks. Every GM melodic program and percussion key receives a musical role across
  four family surveys, then fifteen voices join four themes in a contrapuntal finale.
...
- **Composition engines** are per-album Python using only the standard library — no
  third-party dependencies. Fixed RNG seeds and per-album verifiers support
  repeatable rebuilds.
...
- **[ferrosintesis](crates/ferrosintesis/README.md)** is a Rust MIDI-to-WAV
  synthesizer with no third-party Rust code dependencies. Its default feature
  embeds first-party asset crates containing separately licensed recordings; an
  LA-synthesis layer crossfades recorded attacks into modeled bodies or sustains.
  The voice design was tuned while rendering the Fable 5 albums. It supports all
  128 GM melodic program numbers through shared synthesis families, but it is not
  a certified GM reference implementation.
...
The installable renderer is published as
[`ferrosintesis-cli`](https://crates.io/crates/ferrosintesis-cli); its binary is
named `ferrosintesis`.
...
The album briefs ask for new material using broad traits of named idioms rather
than reproductions of named works. The repository has not established a method
that could guarantee no incidental resemblance to every existing composition.
Individual album READMEs describe their references and any deliberate internal
reuse.
