#!/usr/bin/env python3
"""Build or verify the fourteen original pieces in *Atlas of Becoming*."""
...
def build_payloads(track_number: int | None = None) -> tuple[list[dict[str, object]], dict[str, bytes]]:
    ...
    for spec in selected_specs(track_number):
        ...
        data = score.to_bytes(spec.title, COMMENT)
        ...
        midi_payloads[spec.filename] = data
    ...
    return built, {**midi_payloads, "album_manifest.json": manifest_bytes}


def write_outputs() -> None:
    built, payloads = build_payloads()
...
def verify_outputs(track_number: int | None = None) -> None:
    ...
    if track_number is None:
        ...
        if not path.exists():
            failures.append(f"{path} missing; run python build.py first")
        elif path.read_text(encoding="utf-8") != payloads["album_manifest.json"].decode("utf-8"):
            failures.append(f"{path} is stale; rebuild before verifying")

    results = verify.run_all([(item["spec"], item["score"]) for item in built], suite=track_number is None)
