#!/usr/bin/env python3
"""build.py — render or verify *Big Weather* (ten tracks).

    python build.py                     rebuild all 10 MIDIs into midi/
                                        + album_manifest.json
    python build.py --track N           rebuild only track N (no manifest
                                        write); prints its section timing
                                        table
    python build.py --verify            rebuild every Score in memory,
                                        re-parse every written MIDI, run
                                        verify.run_track() per track;
                                        prints a pass/fail table and exits
                                        nonzero on any failure
    python build.py --track N --verify  verify ONLY track N — the
                                        composer's fast loop
...
this file only iterates conductor.REGISTRY / movements.load_tracks() and
applies the shared machinery.  Per-track seeds are fixed so a rebuild is
byte-identical and --verify reasons about the same Scores that produced
the committed files.

Composer's loop reminder (repo lesson): edit -> `--track N` (WRITES the
MIDI) -> `--track N --verify` -> render -> analyze, in that order —
`--verify` alone never rewrites the file.
"""
...
def _default_comment(module) -> str:
    return (f"Track {module.NUMBER:02d} of '{conductor.ALBUM}', a rock/pop "
            f"album composed and rendered by {conductor.ARTIST}.")


def build_score(module) -> tuple[en.Score, list]:
    """Build one track's full Score; returns (score, per-section spans).

    Runs module.PART.setup then each of the module's section builders in
...
def verify_tracks(mods, in_memory_only: bool = False) -> None:
    """Run every oracle for each module."""
