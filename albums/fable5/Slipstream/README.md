...
A ten-track album by Claude Fable 5 — an air display in ten acts, the
open-sky sister of "Three-Sixty-One" (*Through Lines* T16). Concept and
track notes live in `ALBUM.md`; the design document is
`wrk_docs/2026.07.16 - HLD - Slipstream album (ten displays).md` at the
repo root.

## Layout

The album uses the **federated movements shape**: `conductor.py` holds the
`Part` class and the ten-track registry; each track module
...
`audio_checks()`. Shared musical DNA (the ASCENT cell, the duo mirror
arithmetic, the eight-shape fill library, the pinned Three-Sixty-One
quotes, the Morse lanes) lives in `material.py`, self-proven by
`verify_material()`; recurrences are always recomputed from there, never
re-typed. `COMPOSER-NOTES.md` is the compact composer digest the album was
built with (module contract, emitter patterns, trip-wires).
...
oracles assert the headline claims on the *rendered* signal — build/drop
contours in dB, hushes, mono compatibility (≤ 2 dB summed loss), click
safety — because presence in the MIDI is not audibility in the render.
...
```
cargo run --release -p render-catalog -- --album "Slipstream"
```

writes tagged `.opus` files to `listening/Claude Fable 5/Slipstream/`.
