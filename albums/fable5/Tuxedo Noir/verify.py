"""verify.py — structural oracles for *Tuxedo Noir* (one track).

The spy-specific requirements: the meter tour is exact (4/4 swing ->
12/8 -> 7/8 -> 4/4), echo throws on the twang theme, the velvet
fiddle's portamento window, the showdown dive's RPN-12 window with a
hard reset, the choir summit vowel, and a dynamics arc whose drops
(Velvet, Last Cigarette) sit below the Showdown peak.
"""
...
NOTE_RANGES = {
    ...
    CH.CH_FLUTE:   (55, 103),
    CH.CH_FIDDLE:  (55, 100),
    CH.CH_CELESTA: (60, 108),
    CH.CH_BELL:    (45, 92),
}

PART_CONFIG = {
    1: dict(
        duration=(170.0, 215.0),
        timesig_grid=[(0.0, 4, 4), (144.0, 12, 8), (204.0, 7, 8),
                      (274.0, 4, 4)],
        keysig_grid=[(0.0, 1, 1)],
        min_tempo_events=5,
        cc_inventory=[
            (CH.CH_TWANG,  94,  16.0,  96.0, 4, None,
             "vamp theme echo throws"),
            (CH.CH_FIDDLE,  1, 144.0, 204.0, 6, None,
             "velvet fiddle vibrato"),
            (CH.CH_SOLO,    1, 204.0, 274.0, 2, None,
             "chase solo vibrato"),
            (CH.CH_STRINGS, 11, 80.0,  96.0, 4, None,
             "vamp string swell"),
        ],
        vowel_sections=[(CH.CH_CHOIR, 316.0, 354.0, "chorus")],
        portamento=[(CH.CH_FIDDLE, 144.0, 204.0, 206.0)],
        rpn_windows=[(CH.CH_SOLO, 314.0, 338.0, 12.0)],
        rpn_reset_by={CH.CH_SOLO: 338.0},
        fine_tune_expect=[(CH.CH_SAW, 92.0, 98.0, 2.0, 8.0)],
        bend_recenter=[16.0, 96.0, 144.0, 204.0, 274.0, 354.0, 392.0],
        aftertouch_min=[(CH.CH_PAD, 30), (CH.CH_CHOIR, 25)],
        vel_chain=["Cold Open", "The Vamp", "Stabs", "Showdown"],
        quieter_than_peak=["Velvet", "Last Cigarette"],
        density_peak="Showdown",
    ),
}
