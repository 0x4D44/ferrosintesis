...
### Fix summary (2026-07-27, deltic:auto run=fix-20260727T121001Z-p9812-n920194600-c80 code=f819709b8b7ae82e4bfbc63c28086be4ece97b0e gate=focused+render-diff)

Agent-reported summary: Fixed MM-BUG-KILN-00016 by making GM36/37 slap and slap-pop identity depend on the in-loop fret collision rather than the retained post-output burst. The fail-first regression reproduced the verifier's recorded doubt: before the fix, SLAP's removed post-output click carried 0.00040660 high-band burst energy while the measured string-contact residual carried only 0.00004923. The code now lowers the legacy post-output click gains to secondary-detail levels while leaving the fret-contact engine, routing, and non-slap presets unchanged. The new regression lesions only the click while retaining `fret_contact`, proves the contact-only voice remains distinct from fingered bass, and bounds the removed burst below 35% of contact energy.
...
- cargo test -p ferrosintesis voices::tests::coupled_loop_margin_holds -- --nocapture passed
- cargo test -p ferrosintesis voices::tests::v2_untouched_pluck_signatures_are_stable -- --nocapture passed
- cargo test -p ferrosintesis voices::tests::default_bass_family_held_levels_stay_together -- --nocapture passed
- Full `albums/**/*.mid` render diff at 11,025 Hz: 0 expected changed, 124 expected same, 0 contamination, 0 not reached
- Full `demos/**/*.mid` render diff at 11,025 Hz: 4 expected changed, 13 expected same, 0 contamination, 0 not reached
