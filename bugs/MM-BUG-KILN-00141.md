...
- **State history:** Open (2026-07-26, raised via `deltic bugs new` model=claude-opus-5@high) -> Fixed (2026-07-26, deltic:auto role=fix run=fix-20260726T150650Z-p48908-n149273900-c1 branch=task/bug-MM-BUG-KILN-00141-run-fix-20260726T150650Z-p48908-n149273900-c1 code=6ec035d9b4c09e345c071037ee58a7fcca266aa5 gate=manual) -> Closed (2026-07-26, verified by Claude Opus 5 @ high, fresh context - I raised this bug out of the MM-BUG-KILN-00134 verification but did not fix it (fixer: deltic:auto role=fix), so I am eligible as the second pair of eyes, on the same footing as the MM-BUG-KILN-00110 closure. Repo gate green on the fix-bearing tree: `cargo fmt --all --check`, both clippy configurations with `-D warnings`, and both test suites - 1486 tests, 0 failures; the sample-tool Python suite passes 75, up from 69. Both halves of this report are delivered, and the second half is the one that matters. FIRST, the helper itself: `ensure_ydp_sf2` (`tools/ferrosintesis-samples/prepare.py:2641`) no longer guards on existence - it calls `cached_members_match(src, YDP_URL, YDP_SHA256, ...)`, the same content-addressed manifest predicate the Salamander and archive helpers use, and rebuilds through `rebuild_ydp_cache` when the manifest does not hold. I verified the behaviour by driving the real function rather than reading it, with a synthetic pinned archive and a stubbed fetch: a cold build fetches once and yields the pinned bytes; a warm, good cache is reused with NO refetch, so the fix does not re-download ~36 MB to authenticate a healthy cache; and the bug's own case - substituting the cached SF2 with different bytes of identical length - is REJECTED AND RESTORED to the pinned content, again without a refetch since the cached archive still matched its pin. SECOND, and this is what ends the class rather than the instance: the fix added the derived cross-helper oracle this report asked for. `test_every_pinned_ensure_helper_authenticates_its_warm_cache` (`test_prepare.py:901`) parses `prepare.py` with `ast` and enumerates the pinned `ensure_*` helpers from the syntax tree - not from a hand-written list, which is the trap that let this defect recur three times (MM-BUG-KILN-00062, then 00134, then this). It recognises authentication through `cached_members_match`/`sha256_file`, resolves it TRANSITIVELY so a helper that delegates to another still counts, and specifically detects the defect's shape: a hash comparison hidden inside an `if not os.path.exists(...)` guard. It also carries its own negative control, `test_oracle_rejects_hashing_hidden_behind_a_missing_file_guard`, which feeds a synthetic helper with exactly that shape and requires the oracle to flag it - CLAUDE.md's "write the adversarial document that should fail your oracle, and check that it does". I proved it non-vacuous against the REAL source, not just the synthetic fixture: run against the committed `prepare.py` it reports no unauthenticated helpers, which independently reproduces the manual census I did when raising this bug; run against a copy with `ensure_ydp_sf2` reverted to its pre-fix shape it returns exactly `['ensure_ydp_sf2']`. So a future pinned helper that hides its check behind an existence guard is now caught automatically. No residual.)
