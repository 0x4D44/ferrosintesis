...
[ferrosintesis](https://github.com/0x4D44/ferrosintesis) — crash, splash and an
18" china, mono 16-bit 44.1 kHz, from two CC0 1.0 sources.
...
ring for fractions of one, so these banks held much of the payload; moving them
keeps both packages below the limit with no change to the audio. A render
...
Everything is **CC0 1.0** (public domain): Virtuosity Drums for the crash and
