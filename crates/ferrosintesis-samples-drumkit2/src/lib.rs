...
mod tests {
    ...
    fn lookup_misses_do_not_initialize_pcm_cache() {
        ...
        assert!(
            output.status.success() && stdout.contains("FERRO_DRUMKIT2_PCM_MISS_CHILD_RAN"),
            "the isolated PCM lookup probe did not provide child evidence:\n{stdout}\n{}",
            String::from_utf8_lossy(&output.stderr),
        );
        println!("FERRO_DRUMKIT2_PCM_MISS_CHILD_RAN");
    }
    ...
    fn inherited_probe_marker_still_runs_the_isolated_child() {
        ...
        .env("FERRO_DRUMKIT2_PCM_MISS_PROBE", "inherited")
        .output()
    ...
    fn lookup_misses_do_not_initialize_pcm_cache_child() {
        assert_eq!(
            std::env::var("FERRO_DRUMKIT2_PCM_MISS_PROBE").as_deref(),
            Ok("child"),
