...
44.1 kHz FLACs (1.11 MiB aggregate), a soft + loud velocity pair. Generated by
...
committed source WAVs under `tools/ferrosintesis-samples/gong-src/`. The FLACs
under `samples/` are **committed packaged outputs** and are embedded into the
synth binary via `include_bytes!`.
...
records for `gong_ageng_loud.flac` in the inventory below. That is a cross-check between the
...
|---|---|---|---|
| `gong_ageng_soft.flac` | 261890 | 13.640 s | 518,201 |
| `gong_ageng_loud.flac` | 261893 | 20.049 s | 642,435 |

Aggregate: 1,160,636 bytes (2 files). Pinned by the `EXPECTED_BYTES` constant
in `src/lib.rs` and the `inventory_matches_packaged_flac_samples` parity test.
...
multi-second bloom is the whole instrument. Output: FLAC containing 16-bit mono
PCM. One-shots
