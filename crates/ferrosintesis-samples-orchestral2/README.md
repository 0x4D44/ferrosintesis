...
Each non-banjo FLAC is a mono 16-bit 44.1 kHz attack transient trimmed by
`tools/ferrosintesis-samples/prepare.py`; the banjo FLACs are extracted by
`tools/ferrosintesis-samples/banjo_extract.py`. The synth plays each FLAC as the
...
every shipped FLAC family, file count, instrument, source pin or owner-recorded
...
FLACs live under `tools/ferrosintesis-samples/` and `samples/`, but consumers and
