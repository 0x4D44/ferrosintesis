# ferrosintesis-samples-vcsl-steinway

Embedded **CC0** VCSL "Grand Piano, Steinway B" attack/body samples — a warm,
intimate vintage Steinway that voices a **GM 0 Acoustic Grand alternate**
(CC0=3) in [ferrosintesis](https://github.com/0x4D44/ferrosintesis). A tonal
contrast to the bright Salamander C5 grand (CC0=2).

9 pitch zones (C2–C6) × 3 dynamics (pp/mf/f) × 2 round robins = 54 logical
sample names backed by 27 unique mono 16-bit 44.1 kHz FLACs. The 27 names that
reuse an upstream velocity layer resolve through the packaged `ALIASES` manifest,
so each payload is embedded and decoded once. Consumers normally reach this crate
through `ferrosintesis`. Provenance and processing: see `PROVENANCE.md`. Licence:
CC0 1.0 (public domain); `NOTICE` carries a courtesy credit to VCSL / Sam Gossner.

Regenerate the FLACs with `python3 tools/ferrosintesis-samples/prepare.py --only=steinwayb`.
