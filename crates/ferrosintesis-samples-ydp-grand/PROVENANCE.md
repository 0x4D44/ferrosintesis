...
(bank select CC0=1): 9 pitch zones (C2–C6, C/F-sharp roots every six semitones) × single velocity, mono 16-bit 44.1 kHz
FLACs. Generated by `tools/ferrosintesis-samples/prepare.py` (`_bake_ydp_grand`)
from the FreePats YDP Grand SoundFont. The FLACs under `samples/` are **packaged data,
