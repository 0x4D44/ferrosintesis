...
CH = 9
_STEP = 2.0            # beats between key hits
_CHOKE_AFTER = 1.3
...
def total_beats() -> float:
    # roll-call + brush + choke + velocity sweep, all laid out below.
    n_roll = len(DRUM_KEYS)
    n_brush = len(BRUSH_KEYS)
    return (n_roll + n_brush) * _STEP + 8.0 + 8.0 + TAIL_BEATS
...
def build(sc: en.Score) -> None:
    ...
    # V3 kit roll-call: one isolated hit per key, choked before the next.
    for key in DRUM_KEYS:
        sc.marker(t, f"KEY {key:03d} {DRUM_NAMES.get(key, '?')}")
        _hit(sc, key, t + 0.5)
        sc.cc(CH, 120, 0, t + _CHOKE_AFTER + 0.5)
        t += _STEP

    # Brush kit: program 40 selects it (drums.rs:1342); any other program resets V3.
    sc.program(CH, 40, t)
    for key in BRUSH_KEYS:
        ...
        sc.cc(CH, 120, 0, t + _CHOKE_AFTER + 0.5)
        t += _STEP
    sc.program(CH, 0, t)   # back to V3
    t += 1.0

    # Hi-hat choke group: 42/44/46 choke each other (engine.rs:1078). Open then closed.
