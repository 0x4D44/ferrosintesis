"""verify.py — structural oracles for *The Burning Meridian* (three
tracks).  Adapted from the Heliopause suite; the epic-specific
requirements: exact meter grids (12/8, 3/4, 5/4 + the 4/4 turns), the
D-major key-signature turn in track 3, horn-section fine-tune spreads,
the cello portamento window, choir vowel arcs, and per-track dynamic
arcs with real drops.
"""
...
PART_CONFIG = {
    1: dict(
        duration=(185.0, 230.0),
        timesig_grid=[(0.0, 4, 4), (32.0, 12, 8), (356.0, 4, 4)],
        keysig_grid=[(0.0, -1, 1)],
        min_tempo_events=3,
        cc_inventory=[
            (CH.CH_STRINGS, 11,  32.0, 176.0, 10, "rise",
             "T1 string swell arc"),
            (CH.CH_FIDDLE,   1, 356.0, 428.0,  4, None,
             "T1 fiddle vibrato wheel"),
            (CH.CH_PAD,     74, 404.0, 428.0,  4, "fall",
             "T1 pad close"),
        ],
        vowel_sections=[(CH.CH_CHOIR1, 104.0, 176.0, "hum"),
                        (CH.CH_CHOIR1, 246.0, 356.0, "chorus")],
        portamento=[],
        rpn_windows=[],
        rpn_reset_by={},
        fine_tune_expect=[(CH.CH_HORN2, 190.0, 200.0, 3.0, 9.0)],
        bend_recenter=[356.0, 428.0],
        aftertouch_min=[(CH.CH_PAD, 30), (CH.CH_CHOIR1, 30)],
        vel_chain=["Embers", "The Ostinato", "The Call"],
        quieter_than_peak=["Over the Hill"],
        density_peak="The Call",
    ),
    2: dict(
        duration=(190.0, 235.0),
        timesig_grid=[(0.0, 3, 4)],
        keysig_grid=[(0.0, 0, 1)],
        min_tempo_events=3,
        cc_inventory=[
            (CH.CH_FIDDLE, 1,  36.0, 132.0, 8, None,
             "T2 fiddle vibrato in the duet"),
            (CH.CH_PIANO, 64,  36.0, 132.0, 2, None,
             "T2 piano pools"),
            (CH.CH_PAD,   74, 240.0, 288.0, 4, "fall",
             "T2 ashfall filter close"),
        ],
        vowel_sections=[(CH.CH_CHOIR1, 132.0, 216.0, "chorus"),
                        (CH.CH_CHOIR1, 226.0, 288.0, "hum")],
        portamento=[],
        rpn_windows=[],
        rpn_reset_by={},
        fine_tune_expect=[],
        bend_recenter=[288.0],
        aftertouch_min=[(CH.CH_PAD, 30), (CH.CH_CHOIR1, 30)],
        vel_chain=["Lanterns", "Duet", "Swell"],
        quieter_than_peak=["Ashfall"],
        density_peak="Swell",
    ),
    3: dict(
        duration=(178.0, 235.0),
        timesig_grid=[(0.0, 5, 4), (160.0, 4, 4), (200.0, 5, 4),
                      (340.0, 4, 4)],
        keysig_grid=[(0.0, -1, 1), (340.0, 2, 0)],
        min_tempo_events=4,
        cc_inventory=[
            (CH.CH_CELLO,  5, 160.0, 200.0, 1, None,
             "T3 cello portamento time"),
            (CH.CH_FIDDLE, 1, 160.0, 200.0, 4, None,
             "T3 fiddle vibrato in the break"),
        ],
        vowel_sections=[(CH.CH_CHOIR1, 283.0, 340.0, "chorus")],
        portamento=[(CH.CH_CELLO, 160.0, 200.0, 202.0)],
        rpn_windows=[],
        rpn_reset_by={},
        fine_tune_expect=[(CH.CH_HORN2, 60.0, 66.0, 3.0, 9.0)],
        bend_recenter=[160.0, 200.0, 340.0, 416.0],
        aftertouch_min=[(CH.CH_PAD, 30), (CH.CH_CHOIR1, 30)],
        vel_chain=["War Footing", "Cavalry", "Charge"],
        quieter_than_peak=["The Break"],
        density_peak="Charge",
    ),
}
