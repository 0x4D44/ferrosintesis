#!/usr/bin/env python3
"""analyze.py — measured audio verification for *Winter Guests*
(roadmap section 7).  Standalone, standard library only.  Run AFTER
rendering a part's MIDI to a 16-bit stereo WAV:

    python analyze.py "wav/01 - Winter Guests, Part One.wav"
                      [--manifest album_manifest.json] [--track N]
                      [--json] [--click-threshold 0.4] [--silence-db -60]

The album has TWO tracks: the movement boundaries are taken from the
manifest track whose file name matches the WAV's name (or the track
forced with --track N).

Per movement (boundaries in seconds from album_manifest.json) it prints:

  RMS (dBFS)          the dynamics arc, measured (P1 peaks in M3, P2 in
                      M5; both parts end quiet)
  stereo correlation  width (+1 mono ... 0 decorrelated); the M5 lead-
                      double section should be wider (lower) than M6
  spectral centroid   one 2048-sample FFT window per second (hand-rolled
                      iterative radix-2 FFT); the per-movement trend —
                      M1's CC74 filter opening should read as rising,
                      M6's closing as falling
...
a minute of runtime for a 9-minute 44.1 kHz stereo file.
"""
...
def select_track(manifest: Path, wav: Path,
                 track_number: int | None) -> dict:
    """Pick the manifest track for this WAV: --track N wins; otherwise the
    track whose MIDI file stem matches the WAV stem (falling back to the
    leading 'NN' number prefix)."""
    ...
    if track_number is not None:
        for track in tracks:
            if track["number"] == track_number:
                return track
        raise SystemExit(f"ERROR: no track {track_number} in "
                         f"{manifest.name} (has "
                         f"{[t['number'] for t in tracks]})")
    stem = wav.stem.lower()
    hits = [t for t in tracks if Path(t["file"]).stem.lower() == stem]
    if not hits:
        prefix = stem.split(" ")[0]
        hits = [t for t in tracks
                if Path(t["file"]).stem.lower().split(" ")[0] == prefix]
    if len(hits) != 1:
        raise SystemExit(f"ERROR: cannot match {wav.name} to exactly one "
                         f"manifest track ({len(hits)} matches) - "
                         f"use --track N")
    return hits[0]


def movements_of(track: dict) -> list[tuple[str, float, float]]:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Measured audio verification for Winter Guests "
                    "(roadmap section 7).")
    ...
    parser.add_argument("--track", type=int, default=None,
                        help="manifest track number (default: match the "
                             "WAV file name against the manifest tracks)")
    ...
    if args.manifest.exists():
        track = select_track(args.manifest, args.wav, args.track)
        track_title = track.get("title")
        movements = movements_of(track)
    else:
        movements = []
    if not movements:
    ...
    if args.as_json:
        ...
        title = f" ({track_title})" if track_title else ""
        print(f"{args.wav.name}{title}: {duration:.1f}s @ {rate} Hz")
        print(f"{'movement':<24}{'span':>16}{'RMS dBFS':>10}"
              f"{'corr':>7}  centroid")
        for mv in movement_stats:
            ...
            print(f"{mv['name']:<24}{span:>16}{rms:>10}{corr:>7}  {cent}")
