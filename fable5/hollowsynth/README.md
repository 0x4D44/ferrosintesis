...
| **SawStack** | detuned polyBLEP saw ensemble — **each layer with its own vibrato rate/phase and a slow random pitch drift**, so a section sounds like players, not one detuned synth → lowpass (strings, pads; the sweep pad's filter is LFO-swept) or → **vocal formant bank** that morphs open at the onset ("mm-ah") | strings 48–51, choir 52–54, pads 88–95 |
