...
def main():
    ...
    shipped_docs = [
        name
        for name in ("LICENSE-CC0", "LICENSE-MIT", "NOTICE", "PROVENANCE.md")
        if os.path.isfile(os.path.join(crate, name)) and name in manifest
    ]
    if not shipped_docs:
        raise SystemExit(
            f"{crate} packages no legal/provenance document; fix Cargo.toml before generating"
        )
    docs = " / ".join(f"`{name}`" for name in shipped_docs)
    ...
    lines.append(f"//! `ferrosintesis`. Licence/provenance: see {docs}.")
    lines.append("")
