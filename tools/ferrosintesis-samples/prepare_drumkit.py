...
def publish_staged(staging_root, repo_root, plans):
    """Encode every staged take into its destination bank, then swap them in.

    The plan speaks LOGICAL names (`kick_vl1_rr1.wav`), which is what staging
    holds and what the recipe tables and provenance documents name. The published
    bank is FLAC, so the encode happens on this boundary -- the one place that
    ...
    try:
        for package, expected in plans.items():
            ...
            for name in sorted(expected):
                ...
                _encode_flac(source, part)
                if _decode_flac_pcm(part) != _read_wav_pcm(source):
                    raise RuntimeError(f"{name}: FLAC encode was not bit-exact")
