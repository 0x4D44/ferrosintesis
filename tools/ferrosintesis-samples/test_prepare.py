...
class MuseScoreOnsetConcurrentExtractionTest(unittest.TestCase):
    ...
    def test_two_processes_use_isolated_onset_decode_intermediates(self):
        worker = textwrap.dedent(
            """
            import os
            import sys
            ...
            def fake_run(command, check):
                ...
                if ogg.endswith("sitar_40.ogg"):
                    ...
                    while not os.path.exists(os.path.join(sync, f"ready-{other}")):
                        if time.monotonic() >= deadline:
                            raise RuntimeError("concurrent onset barrier timed out")
                        time.sleep(0.01)
            ...
            with (
                ...
                prepare._bake_sf_onset(
                    src, 104, "sitar", "ferrosintesis-samples-musescore",
                    0.9, 0.20, expected_roots=roots)
            """
        )
        with tempfile.TemporaryDirectory() as temp_root:
            src = os.path.join(temp_root, "shared-cache")
            ...
            sync = os.path.join(temp_root, "sync")
            os.makedirs(src)
            os.makedirs(os.path.join(repo_root, "crates", "ferrosintesis-samples-musescore", "samples"))
            os.makedirs(sync)
            prior_bank = os.path.join(
                repo_root, "crates", "ferrosintesis-samples-musescore", "samples",
                "sitar_C4.flac")
            prior_bytes = b"prior-valid-sitar-bank"
            with open(prior_bank, "wb") as output:
                output.write(prior_bytes)
            with open(os.path.join(src, "MS_Basic.sf3"), "wb") as source:
                source.write(b"authenticated synthetic sf3")
            script = os.path.join(temp_root, "concurrent_onset_worker.py")
            with open(script, "w", encoding="utf-8") as output:
                output.write(worker)
            ...
            try:
                ...
                for process in processes:
                    process.communicate()
                self.fail("concurrent MuseScore onset worker timed out")
            ...
            with open(os.path.join(sync, "path-A"), encoding="ascii") as path:
                ogg_a, wav_a = path.read().splitlines()
            with open(os.path.join(sync, "path-B"), encoding="ascii") as path:
                ogg_b, wav_b = path.read().splitlines()
            ...
            with open(prior_bank, "rb") as source:
...
class MandolinWholeBankPublicationTest(unittest.TestCase):
    """MM-BUG-CRU-00048: failed mandolin bakes preserve the previous bank."""
    ...
    def setUp(self):
        ...
        self.mandolin_sources = {
            name: f"fixture/{name}" for name in self.expected
        }
        for name in self.expected:
            packaged = prepare.packaged_name(name)
            with open(os.path.join(self.sample_dir, packaged), "wb") as output:
                output.write(f"old mandolin bank: {packaged}".encode("ascii"))
    ...
    def run_bake(self, transform):
        with (
            mock.patch.object(prepare, "REPO_ROOT", self.repo_root),
            mock.patch.object(prepare, "MANDOLIN_SOURCES", self.mandolin_sources),
            mock.patch.object(
                prepare, "_prepare_generic_source_sample", side_effect=transform
            ),
        ):
            return prepare._bake_mandolin(self.src)
    ...
    def assert_no_transaction_artifacts(self):
        self.assertEqual(
            [
                ...
                for name in os.listdir(self.sample_dir)
                if name.endswith((".wav", ".part"))
                or name.startswith(".mandolin-backup-")
            ],
            [],
        )
    ...
    def test_late_transform_failure_never_touches_the_previous_bank(self):
        ...
        def fail_on_third(fn, src):
            ...
            calls += 1
            if calls == 3:
                raise OSError("late mandolin transform")
            return self.staged_sample(fn, src)

        with self.assertRaisesRegex(OSError, "late mandolin transform"):
            self.run_bake(fail_on_third)
    ...
    def test_publication_failure_rolls_back_every_selected_file(self):
        ...
        def fail_during_publication(source, destination):
            ...
            replacements += 1
            if replacements == fail_at:
                raise OSError("injected mandolin publication failure")
            return real_replace(source, destination)
        ...
        with (
            ...
            with self.assertRaisesRegex(
                OSError, "injected mandolin publication failure"
            ):
                self.run_bake(self.staged_sample)
...
class SoundfontOnsetWholeBankPublicationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.makedirs(self.sample_dir)
        self.logical_names = tuple(
            f"{self.PREFIX}_{prepare._midi_name(root)}.wav" for root in self.ROOTS
        )
        self.packaged_names = tuple(
            prepare.packaged_name(name) for name in self.logical_names
        )
        for name in self.packaged_names:
            pathlib.Path(self.sample_dir, name).write_bytes(
                b"old-soundfont-onset-bank-" + name.encode()
            )
        self.sf3_path = os.path.join(self.src, "MS_Basic.sf3")
        pathlib.Path(self.sf3_path).write_bytes(self.fake_sf3(self.ROOTS))
    ...
    @classmethod
    def fake_sf3(cls, roots):
        """Build the smallest parsed SF3 fixture with one zone per root."""
        smpl = b"".join(bytes((65 + i,)) * 4 for i, _ in enumerate(roots))
        ...
        def preset(name, number, first_bag):
            record = bytearray(38)
            record[:len(name)] = name
            record[20:22] = struct.pack("<H", number)
            record[22:24] = struct.pack("<H", 0)
            record[24:26] = struct.pack("<H", first_bag)
            return bytes(record)
        ...
        for i, root in enumerate(roots):
            ...
            record[20:40] = struct.pack("<IIIII", start, end, start, end, 44100)
            record[40] = root
            record[44:46] = struct.pack("<H", 17)
            shdr.append(bytes(record))
        shdr.append(bytes(46))

        phdr = preset(b"Sitar", cls.PRESET, 0) + preset(b"terminal", 0, 1)
        pbag = struct.pack("<HH", 0, 0) + struct.pack("<HH", 1, 0)
        pgen = struct.pack("<HH", 41, 0)
        inst = instrument(b"Sitar", 0) + instrument(b"terminal", len(roots))
        ibag = b"".join(
            struct.pack("<HH", index, 0) for index in range(len(roots) + 1)
        )
        igen = b"".join(
            struct.pack("<HH", 53, sample_id) for sample_id in range(len(roots))
        )
        ...
        pdta = list_chunk(
            b"pdta",
            [
                cls.chunk(b"phdr", phdr),
                cls.chunk(b"pbag", pbag),
                cls.chunk(b"pgen", pgen),
                cls.chunk(b"inst", inst),
                cls.chunk(b"ibag", ibag),
                cls.chunk(b"igen", igen),
                cls.chunk(b"shdr", b"".join(shdr)),
    ...
    def run_bake(self, measure):
        ...
        with (
            mock.patch.object(prepare, "REPO_ROOT", self.repo_root),
            mock.patch.object(prepare, "ensure_musescore_sf3", return_value=self.sf3_path),
            mock.patch.object(prepare.shutil, "which", return_value="ffmpeg"),
            mock.patch.object(prepare.subprocess, "run", side_effect=self.decode_fixture),
            mock.patch.object(
    ...
    def test_late_transform_failure_never_touches_the_previous_bank(self):
        ...
        def fail_on_third(*args):
            ...
            calls += 1
            if calls == 3:
                raise OSError("late soundfont onset transform")
            return self.measure_fixture(*args)

        with self.assertRaisesRegex(OSError, "late soundfont onset transform"):
            self.run_bake(fail_on_third)
    ...
    def test_replacement_failure_rolls_back_the_previous_bank(self):
        ...
        def fake_encode(_wav, flac):
            pathlib.Path(flac).write_bytes(b"new-soundfont-onset-flac")

        def fail_during_publication(source, destination):
            nonlocal replacements
            replacements += 1
            if replacements == fail_at:
                raise OSError("injected soundfont onset publication failure")
            return real_replace(source, destination)
        ...
        with (
            ...
            with self.assertRaisesRegex(
                OSError, "injected soundfont onset publication failure"
            ):
                self.run_bake(self.measure_fixture)
...
class HonkytonkSampleApiContractTest(unittest.TestCase):
    ...
    def test_docs_and_lookup_oracle_match_packaged_flac_inventory(self):
        crate = pathlib.Path(prepare.REPO_ROOT) / "crates" / "ferrosintesis-samples-honkytonk"
        samples = sorted(
            path
            for path in (crate / "samples").iterdir()
            if path.is_file() and path.suffix in {".wav", ".flac"}
        ...
        readme = (crate / "README.md").read_text(encoding="utf-8")
        provenance = (crate / "PROVENANCE.md").read_text(encoding="utf-8")
        notice = (crate / "NOTICE").read_text(encoding="utf-8")
        lib = (crate / "src" / "lib.rs").read_text(encoding="utf-8")
        ...
        self.assertEqual(int(file_count.group(1)), len(samples))
        self.assertEqual(int(expected_bytes.group(1)), total)
        self.assertIn(f"{total:,} bytes (9 files)", provenance)
        self.assertIn("raw FLAC bytes", readme)
        self.assertIn("raw FLAC bytes", provenance)
        self.assertNotIn("WAVs", readme)
        self.assertIn("FLAC samples", notice)

        embedded = set(re.findall(r'^\s*"([^"]+\.flac)",\s*$', lib, re.M))
        included = set(re.findall(r'include_bytes!\("\.\./samples/([^"]+)"\)', lib))
        documented = set(re.findall(r"honkytonk_[A-G](?:#)?[2-6]\.flac", readme))
        self.assertEqual(embedded, names)
        self.assertEqual(included, names)
        self.assertEqual(documented, names)
        self.assertIn("pub fn get(name: &str)", lib)
...
class KawaiAliasDeduplicationTest(unittest.TestCase):
    ...
    def assert_aliases_cover_logical_sources(self, physical, aliases):
        expected_physical, expected_aliases = prepare._canonicalize_source_aliases(
            prepare._KAWAI_LOGICAL_SOURCES
        )
    ...
    def test_packaged_sources_do_not_repeat_upstream_payloads(self):
        urls = list(prepare.KAWAI_SOURCES.values())
        self.assertEqual(
            len(urls),
            len(set(urls)),
            "KAWAI_SOURCES must name each physical upstream payload once",
        )
    ...
    def test_declared_aliases_exactly_cover_repeated_logical_sources(self):
        ...
        self.assert_aliases_cover_logical_sources(
            prepare.KAWAI_SOURCES,
            prepare.KAWAI_ALIASES,
        )
        self.assertEqual(len(expected_physical), 32)
        self.assertEqual(len(expected_aliases), 16)
        self.assertEqual(
    ...
    def test_missing_alias_is_rejected_by_the_source_derived_oracle(self):
        ...
        incomplete.pop("kawai_C2_mf.wav")
        with self.assertRaises(AssertionError):
            self.assert_aliases_cover_logical_sources(
                prepare.KAWAI_SOURCES,
                incomplete,
...
class KawaiOutputInventoryTest(unittest.TestCase):
    ...
    def test_removed_source_rejects_its_stale_output_before_fetching_or_writing(self):
        sources = dict(prepare.KAWAI_SOURCES)
        stale = next(iter(sources))
        sources.pop(stale)
        ...
        with tempfile.TemporaryDirectory() as repo_root:
            ...
            with mock.patch.object(prepare, "REPO_ROOT", repo_root), mock.patch.object(
                ...
                prepare.sys, "argv", ["prepare.py", "--only=kawai"]
            ):
                with self.assertRaisesRegex(ValueError, re.escape(stale)):
                    prepare.main()
